...
clean_this <- read.csv(paste('/Users/prajwal/Desktop/Columbia/C4SR/',mn_or_bk,'_',year,'/',mn_or_bk,'_',year,'_combine-',doubttype, '4.csv', sep=''))
col1 <- sapply(toupper(clean_this$Full.Address), clean)
col2 <- sapply(toupper(clean_this$H.House.Number), clean)
col3 <- sapply(toupper(clean_this$H.Street.Name), clean)
col4 <- sapply(toupper(clean_this$c_street1), clean)
col5 <- sapply(toupper(clean_this$c_street2), clean)
col6 <- sapply(toupper(clean_this$n_street1), clean)
col7 <- sapply(toupper(clean_this$n_street2), clean)
col8 <- sapply(toupper(clean_this$new.h.city), clean)
col9 <- sapply(toupper(clean_this$new.h.house.number), clean)
col10 <- sapply(toupper(clean_this$new.h.street.name), clean)
col11 <- sapply(toupper(clean_this$unidentified.h.info), clean)

clean_this_after_cleaning <- data.frame(col1,col2,col3,col4,col5,col6,col7,col8,col9,col10,col11)

write.csv(clean_this_after_cleaning, paste('/Users/prajwal/Desktop/Columbia/C4SR/',mn_or_bk,'_',year,'/after cleaning/',mn_or_bk,'_',year,'_after_cleaning_',doubttype,'4.csv', sep=''))
